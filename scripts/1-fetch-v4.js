...
// Import moduli
import { scriviLog, caricaJSON, salvaJSON, contaArticoli, parseJSON, 
         giornoOggi, risolviAgenda, risolviPersonaggio, 
         caricaContatori, limiteSuperato, fasciaDiArticoliAttiva, paroleTarget,
         pausaGemini } from "./lib/utils.js";
...
import { caricaDraft, inizializzaSezioni, safeWriteDraft } from "./lib/draft.js";
import { applicaDecay, aggiornaRelazioni } from "./lib/relations.js";
import { generaChat } from "./lib/chat.js";
import { generaArticolo, generaCommenti } from "./lib/articoli.js";

// Helper locale per log (passa il path)
...
async function main() {
    ...
    // Inizializza moduli
    initNews(apiKeyGnews, newsSource, log);
    initLLM(apiKeyGemini, apiKeyGroq, PROVIDER_STATE_PATH, log);
    await initModels();
    ...
    relazioni = applicaDecay(relazioni, log);
    ...
    const { draft, isNuovoGiorno, oldDraft } = await caricaDraft(DRAFT_PATH, oggiStr, agenda, IMPOSTAZIONI, STILI, log);
    ...
    for (const { voce, tema } of codaArticoli) {
        ...
        const commenti = await generaCommenti(voce, CHI, relazioni, personaggi, parsed.articolo, [], LIMITI, callLLM, log);
        if (commenti.length) {
            await aggiornaRelazioni(CHI, relazioni, personaggi, parsed.articolo, commenti, callLLM, log);
        }
    ...
    const chat = chattaOggi ? await generaChat(CHI, relazioni, personaggi, callLLM, log) : null;
    if (chat) {
...
Rispondi JSON: {"titolo":"...","articolo":"..."}`;
    ...
    const ok = await safeWriteDraft(draft, DRAFT_PATH, TEMP_PATH, BACKUP_PATH, log);
    if (ok) {
        contatori._primoRunOggi = true;
        salvaJSON(CONTATORI_PATH, contatori);
        log(`✅ FASE 1-v2 completata. ${articoliGenerati} nuovi articoli.`);
    } else {
