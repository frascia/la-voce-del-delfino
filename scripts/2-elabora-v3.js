...
/**
 * 2-elabora-v3.js
 * FASE 2 — Nuova architettura v3
 *
 * Legge _draft-v3.json prodotto da 1-fetch-v4.js.
 * Gli articoli sono già completi (testo, commenti, relazioni aggiornate, provider).
 * Questa fase si occupa di:
 * - Validare e normalizzare i dati
 * - Aggiungere auth_info.json e active_config_v3.json
 * - Produrre _articles-v3.json pronto per il deploy
 * - Calcolare il provider LLM usato (gemini/groq/misto/nessuno)
 */
...
function scriviLog(msg) {
    ...
    const riga = `[${ts}] [2-elabora-v3] ${msg}\n`;
...
async function main() {
    scriviLog("⚙️ FASE 2-v3 — Avvio elaborazione...");

    if (!fs.existsSync(DRAFT_PATH)) {
        scriviLog(`ERRORE: ${DRAFT_PATH} non trovato. Eseguire prima 1-fetch-v4.js`);
        process.exit(1);
    }
    ...
    // --- Determina il provider LLM prevalente dagli articoli ---
    let llmProvider = "nessuno";
    const providerCount = {};
    for (const sez of Object.values(sezioni)) {
        for (const art of (sez.articoli || [])) {
            const prov = art.provider;
            if (prov) providerCount[prov] = (providerCount[prov] || 0) + 1;
        }
    }
    const totali = Object.values(providerCount).reduce((a,b) => a+b, 0);
    if (totali === 0) {
        llmProvider = "nessuno";
    } else if (Object.keys(providerCount).length === 1) {
        llmProvider = Object.keys(providerCount)[0];
    } else {
        llmProvider = "misto";
    }
    scriviLog(`🔍 Provider LLM rilevato: ${llmProvider} (${totali} articoli)`);

    // --- Aggiorna active_config_v3.json ---
    if (fs.existsSync(CONFIG_PATH)) {
        ...
        config.site_settings = {
            ...
            llm_provider: llmProvider   // nuovo campo
        };
        fs.writeFileSync(ACTIVE_CONFIG_PATH, JSON.stringify(config, null, 2));
        scriviLog("📄 active_config_v3.json aggiornato.");
    } else {
        scriviLog(`⚠️ ${CONFIG_PATH} non trovato, skip active_config`);
    }
    ...
    for (const [sez, datiSez] of Object.entries(sezioni)) {
        ...
        for (const art of (datiSez.articoli || [])) {
            ...
            articoliNormalizzati.push({
                ...
                provider:       art.provider || null   // conserva il provider per eventuale debug
            });
        }
    ...
    scriviLog(`✅ FASE 2-v3 completata. Articles → ${ARTICLES_PATH}`);
}
