...
async function generaCommenti(voce, CHI, relazioni, personaggi, articolo, commentiPrecedenti) {
...
Scegli 1-3 personaggi che commenterebbero questa notizia in modo coerente col loro carattere e le loro relazioni.
    ...
    if (!articoliAttivi && fs.existsSync(DRAFT_PATH)) {
        ...
        for (const [sez, datiSez] of Object.entries(draftEsistente.sezioni || {})) {
            for (const articolo of (datiSez.articoli || [])) {
                ...
                const rawCommenti    = await generaCommenti(voceRef, CHI, relazioni, personaggi, articolo.articolo, articolo.commenti || []);
                const parsedCommenti = parseJSON(rawCommenti);
                if (parsedCommenti?.commenti?.length) {
                    // Accoda i nuovi commenti a quelli esistenti (max 6 totali)
                    articolo.commenti = [...(articolo.commenti || []), ...parsedCommenti.commenti].slice(0, 6);
